...
export class RestService {
  ...
  private getKeyValue = (key: string) => (obj: Record<string, any>) => obj[key];
  ...
  private params(data: object): HttpParams {
    ...
    if (data) {
      Object.keys(data)
        .forEach((key) => {
          params = params.set(key, this.getKeyValue(key)(data));
        });
    }
    return params;
  }
  ...
  private headers(data: object): HttpHeaders {
    ...
    if (data) {
      Object.keys(data)
        .forEach((key) => {
          headers = headers.set(key, this.getKeyValue(key)(data));
        });
    }
    return headers;
  }
  ...
  private form(data: object): FormData {
    ...
    if (data) {
      Object.keys(data)
        .forEach((key) => form.append(key, this.getKeyValue(key)(data)));
    }
    return form;
  }
  ...
  private encode(data: object): string {
    ...
    if (data) {
      Object.keys(data)
        .forEach((key) => params.set(key, this.getKeyValue(key)(data)));
    }
  ...
  private data(resource: Resource): any {
    ...
    return resource.encode ? this.encode(resource.data as object) : resource.data;
  }
